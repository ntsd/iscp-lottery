...
export class BasicClient {
  ...
  public async sendFaucetRequest(
  ...
  public async sendOffLedgerRequest(
    chainId: string,
    offLedgerRequest: IOffLedger
  ): Promise<void> {
    const request = {
      Request: OffLedger.ToBuffer(offLedgerRequest).toString("base64"),
    };
  ...
  public async sendExecutionRequest(
    chainId: string,
    offLedgerRequestId: string
  ): Promise<void> {
    await this.sendRequestExt<IOffLedgerRequest, null>(
      this.configuration.WaspAPIUrl,
      "get",
  ...
  public async getAddressUnspentOutputs(
    address: string
  ): Promise<ISingleUnspentOutputResponse> {
  ...
  public async unspentOutputs(
    request: IUnspentOutputsRequest
  ): Promise<IUnspentOutputsResponse> {
  ...
  public async sendTransaction(
    request: ISendTransactionRequest
